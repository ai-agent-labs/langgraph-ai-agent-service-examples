...
책 10장 코드 예제는 교육적 단순화를 위해 도구 함수가 `-> str` 을 반환하는
형태로 소개됩니다. 반면 본 레포의 `src/hr_agent/tools.py` 는 실무 권장
방식인 `-> dict` 반환을 사용합니다(구조화된 필드로 LLM이 더 정확히 해석).
두 방식 모두 `@tool` 데코레이터와 호환되며, 책만 따라 하려는 독자는 단순히
`str(result)` 로 감싸는 것으로 동작 차이를 해소할 수 있습니다.

## Ch11과의 차이점

| 구분 | Ch10 (단일 에이전트) | Ch11 (멀티 에이전트) |
|------|---------------------|---------------------|
| 에이전트 수 | 1개 | 3개 (supervisor + 2 전문가) |
